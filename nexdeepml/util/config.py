...
class ConfigBase(ABC):
    ...
    def enable_debug_mode(self) -> ConfigBase:
        ...
        for key, item in self._parameters.items():
            if issubclass(type(item), ConfigBase):
                item.enable_debug_mode()

        return self

    def disable_debug_mode(self) -> ConfigBase:
...
class ConfigParser(ConfigBase):
    ...
    The leaf nodes will be of type ConfigLeaf.
    """
    ...
    def __init__(self, config_dict: Dict = None, leaf_class: ConfigLeaf = None):
        ...
        self.Leaf: ConfigLeaf = leaf_class if leaf_class is not None else ConfigLeaf

        # Create a dictionary to hold the data
        self._parameters: Dict[str, ConfigBase] = {}
    ...
    def __init_helper(self, config_dict: Dict) -> Dict:
        ...
        def helper(value: Any) -> ConfigBase:
    ...
    def get_leaf(self, item: str) -> ConfigLeaf:
    ...
    def _is_flat(self, contain__: bool = False) -> bool:
        ...
        # Check if non of the parameters are leaf of subtype ConfigLeaf, i.e. the instance is flat
        check = all([
            not issubclass(type(self._parameters.get(parameter_name)), ConfigLeaf)
            for parameter_name
            in parameter_names
...
class ConfigLeaf(ConfigBase):
    ...
    def __init__(self, value: Dict):
        ...
        self._parameters = {key: value for key, value in value.items() if not issubclass(type(value), ConfigBase)}
    ...
    def enable_debug_mode(self) -> ConfigLeaf:
        """Enables debug mode, in this instance and all children, where parameters can be accessed with . notation."""
